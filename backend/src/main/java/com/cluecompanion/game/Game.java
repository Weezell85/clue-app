...
public final class Game {
 ...
 public synchronized boolean accuse(UUID actor,Suggestion a){requireTurn(actor);check(phase==Phase.ACTION||phase==Phase.DECISION,"A final accusation cannot be made now");validate(a);boolean correct=new HashSet<>(solution).equals(new HashSet<>(a.cards()));String cards=a.suspect().name()+", "+a.weapon().name()+", "+a.room().name();if(correct){winner=actor;status=Status.FINISHED;events.add(new Event(player(actor).name()+" correctly accused "+cards+". The game is over!",null,null));}else{events.add(new Event(player(actor).name()+" incorrectly accused "+cards+" and is out of the turn order.",null,null));Player p=players.get(turn);players.set(turn,new Player(p.id(),p.name(),p.host(),p.hand(),true));advance();}return correct;}
 public synchronized void pass(UUID actor){requireTurn(actor);check(phase==Phase.ACTION||phase==Phase.DECISION,"The turn cannot be finished now");advance();}
