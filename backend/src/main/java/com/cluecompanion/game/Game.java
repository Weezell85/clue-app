...
public final class Game {
 public enum Status { LOBBY, PLAYING, FINISHED } public enum Phase { ACTION, DISPROVING, DECISION }
 public record Player(UUID id,String name,boolean host,List<Card> hand,boolean eliminated){}
 ...
 private Status status=Status.LOBBY; private Phase phase=Phase.ACTION; private List<Card> solution=List.of(); private int turn; private Suggestion suggestion; private int responder; private UUID winner;
 ...
 public synchronized void suggest(UUID actor,Suggestion s){requireTurn(actor);check(phase==Phase.ACTION,"A suggestion cannot be made now");validate(s);suggestion=s;responder=next(turn);phase=Phase.DISPROVING;events.add(new Event(player(actor).name()+" suggested "+s.suspect().name()+", "+s.weapon().name()+" in the "+s.room().name(),null,null));skipEmpty();}
 public synchronized void respond(UUID actor,Card card){check(phase==Phase.DISPROVING,"No suggestion needs a response");check(players.get(responder).id().equals(actor),"It is not your response");List<Card>m=matches(players.get(responder));if(card!=null)check(m.contains(card),"Reveal a matching card");else check(m.isEmpty(),"You must reveal one matching card");if(card!=null){events.add(new Event(players.get(responder).name()+" showed a card to "+players.get(turn).name(),null,null));events.add(new Event("Revealed card: "+card.name(),players.get(turn).id(),card));phase=Phase.DECISION;}else{responder=next(responder);skipEmpty();}}
 private void skipEmpty(){while(responder!=turn&&matches(players.get(responder)).isEmpty())responder=next(responder);if(responder==turn){events.add(new Event("No player could disprove the suggestion",null,null));phase=Phase.DECISION;}}
 private List<Card> matches(Player p){return p.hand().stream().filter(suggestion.cards()::contains).toList();}
 public synchronized void noRoom(UUID actor){requireTurn(actor);check(phase==Phase.ACTION,"Action is already complete");phase=Phase.DECISION;events.add(new Event(player(actor).name()+" did not make it into a room",null,null));}
 public synchronized boolean accuse(UUID actor,Suggestion a){requireTurn(actor);check(phase==Phase.DECISION,"Complete your action before accusing");validate(a);boolean correct=new HashSet<>(solution).equals(new HashSet<>(a.cards()));events.add(new Event(player(actor).name()+" made a final accusation — "+(correct?"correct!":"incorrect"),null,null));if(correct){winner=actor;status=Status.FINISHED;}else{Player p=players.get(turn);players.set(turn,new Player(p.id(),p.name(),p.host(),p.hand(),true));advance();}return correct;}
 public synchronized void pass(UUID actor){requireTurn(actor);check(phase==Phase.DECISION,"Complete your action before passing");advance();}
 private void advance(){do{turn=next(turn);}while(players.get(turn).eliminated()&&players.stream().anyMatch(p->!p.eliminated()));suggestion=null;phase=Phase.ACTION;events.add(new Event("It is now "+players.get(turn).name()+"'s turn",null,null));}
