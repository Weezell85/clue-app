...
@RestController @RequestMapping("/api/games") public class GameController {
 ...
 public record View(String code,Game.Status status,Game.Phase phase,List<PlayerDto> players,UUID currentPlayerId,UUID responderId,UUID winnerId,List<CardDto> hand,List<CardDto> cards,CardDto turnRoom,List<CardDto> suggestedCards,List<CardDto> solution,List<EventDto> events){}
 ...
 @PostMapping("/{code}/room") public View enterRoom(@PathVariable String code,@RequestHeader("X-Player-Id") UUID id,@RequestBody CardDto room){Game g=service.get(code);g.enterRoom(id,card(room));return view(g,id);}
 ...
 private View view(Game g,UUID id){Game.Player me=g.players().stream().filter(p->p.id().equals(id)).findFirst().orElseThrow(()->new GameException("Unknown player"));return new View(g.code(),g.status(),g.phase(),g.players().stream().map(p->new PlayerDto(p.id(),p.name(),p.host(),p.eliminated(),p.hand().size())).toList(),g.currentPlayer().id(),g.currentResponder()==null?null:g.currentResponder().id(),g.winner(),me.hand().stream().map(this::dto).toList(),Deck.cards().stream().map(this::dto).toList(),dto(g.turnRoom()),g.suggestion()==null?List.of():g.suggestion().cards().stream().map(this::dto).toList(),g.solution().stream().map(this::dto).toList(),g.eventsFor(id).stream().map(e->new EventDto(e.message(),dto(e.privateCard()))).toList());}
}
